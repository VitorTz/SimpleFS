...
class File_Ops {
public:
    static int do_copyin(const char *filename, int inumber, SimpleFs *fs);
    static int do_copyout(int inumber, const char *filename, SimpleFs *fs);
};
...
int File_Ops::do_copyin(const char *filename, const int inumber, SimpleFs *fs) {
	FILE *file;
...
int File_Ops::do_copyout(const int inumber, const char *filename, SimpleFs *fs) {
	FILE *file;
	int offset = 0, result;
	char buffer[16384];
